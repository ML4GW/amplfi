...
poetry run python train/cli/flow.py fit --config configs/flow/cbc.yaml
```

or by running inside the training container

```console
APPTAINERENV_AMPLFI_OUTDIR=$AMPLFI_OUTDIR APPTAINERENV_AMPLFI_DATADIR=$AMPLFI_DATADIR apptainer run --nv $AMPLFI_CONTAINER_ROOT/train.sif python /opt/amplfi/projects/train/train/cli/flow.py fit --config /path/to/config.yaml
```
